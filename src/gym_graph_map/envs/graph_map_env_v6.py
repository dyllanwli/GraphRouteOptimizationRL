...
class GraphMapEnvV6(gym.Env):
    ...
    def _get_nested_embeddings(self, u, embeddings, pre, weight, n=1):
        ...
        """
        embeddings[u] = self.graph[pre][u][0][weight]
        pre_embedding = embeddings[u]
        neighbors = list(x for x in self.graph.neighbors(u) if x not in self.passed_nodes_ids) # get neighbors of u
        for v in neighbors:
            cur_embedding = self.graph[u][v][0][weight] + pre_embedding
            embeddings[v] = min(cur_embedding, embeddings[v]) if embeddings[v] != 0 else cur_embedding
        return embeddings
